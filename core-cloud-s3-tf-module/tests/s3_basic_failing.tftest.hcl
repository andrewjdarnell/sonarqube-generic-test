// Intentionally failing Terraform tests used to validate failure reporting in
// CLI output and generated JUnit XML artifacts.
//
// Expected to fail:
// - validate_bucket_creation (first assertion)
// - validate_kms_key_creation (second assertion)
// - validate_versioning_enabled (assertion)
...
run "validate_bucket_creation" {
  ...
  # Intentional failure for reporting validation.
  assert {
    condition     = aws_s3_bucket.this.bucket == "definitely-wrong-bucket-name"
    error_message = "Intentional failure: bucket name should not match this fake value"
  }
...
run "validate_kms_key_creation" {
  ...
  # Intentional failure for reporting validation.
  assert {
    condition     = aws_kms_key.s3.deletion_window_in_days == 30
    error_message = "Intentional failure: deletion window is expected to differ from this value"
  }
...
run "validate_versioning_enabled" {
  ...
  assert {
    ...
    error_message = "Intentional failure: versioning status is expected not to be Suspended when enabled"
  }
}
